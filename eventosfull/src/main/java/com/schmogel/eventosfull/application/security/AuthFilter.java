...
public class AuthFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
      ...
      if(!authManager.validarRequisicao(servletRequest.getHeader("authorization"), servletRequest.getHeader("client"))){
        servletResponse.setStatus(HttpStatus.UNAUTHORIZED.value());
        servletResponse.getWriter().write("Tentativa de acesso por cliente nao autorizado (API para API)");
        return;
      };

      chain.doFilter(request, response);

      logger.saveLog(new LogInfo(
          null,
